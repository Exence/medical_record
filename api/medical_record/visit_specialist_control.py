...
async def add_extra_class(visit_specialist_control: VisitSpecialistControlCreate, 
                          user: User = Depends(get_current_user), 
                          service: VisitSpecialistControlService = Depends()):
    service.add_new_visit_specialist_control(user=user, visit_specialist_control_data=visit_specialist_control)

@router.post('/update')
async def update_visit_specialist_control(visit_specialist_control: VisitSpecialistControlUpdate, 
                                          user: User = Depends(get_current_user), 
                                          service: VisitSpecialistControlService = Depends()):
    service.update_visit_specialist_control(user=user, visit_specialist_control_data=visit_specialist_control)

@router.post('/delete')
async def delete_visit_specialist_control(visit_specialist_control_pk: VisitSpecialistControlPK, 
                                          user: User = Depends(get_current_user), 
                                          service: VisitSpecialistControlService = Depends()):
    service.delete_visit_specialist_control(user=user, visit_specialist_control_pk=visit_specialist_control_pk)
